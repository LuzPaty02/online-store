...
export const Checkout = () => {
    ...
    useEffect(() => {
        if (address.postalCode && cartItems) {
            const totalWeight = cartItems.reduce((sum, { product }) => sum + (product.weight || 1), 0);
    ...
    useEffect(() => {
        const isValid = address.fullName.trim() !== '' &&
            address.street.trim() !== '' &&
            isShippingSelected;
                    ...
                    <div className="form-group">
                        ...
                        {address.postalCode && (
                            <div className="address-preview">
                                {address.colonia}, {address.city}, {address.municipality}, {address.state}
                            </div>
                        )}
                    </div>
                        ...
                        <div className="shipping-options">
                            ...
                            {shippingOptions.map((option, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className={`shipping-option ${carrierClass} ${selectedShipping?.service === option.service ? 'selected' : ''
                                            }`}
                                    >
                                        <input
                                            type="radio"
                                        ...
                                        <label htmlFor={`shipping-${index}`}>
                                            <div className="shipping-option-content">
                                                ...
                                                <img
                                                    src={carrierClass == 'fedex'
                                                        ?
                                                        'https://e7.pngegg.com/pngimages/115/184/png-clipartlogo-business-brand-fedex-design-activity-room-text-trademark.png' :
                                                        'https://www.citypng.com/public/uploads/preview/upsdelivery-company-logo-symbol-hd-png-701751694777602klzxvtwtqk.png'}
                                                    alt={option.carrier}
                                                ...
                                                <div className="shipping-details">
                                                    ...
                                                    <span className="shippingprice">${option.amount.toFixed(2)}</span>
                                                    <div className="shipping-estimate">
                                                        Estimated
                                                        delivery: {new
                                                            Date(option.estimatedDeliveryDate).toLocaleDateString()}
                                                    </div>
                                                </div>
                                            </div>
                                        </label>
                ...
                <div className="order-summary">
                    ...
                    {isAddressValid ? (
                        ...
                        <div className="payment-requirements-message">
                            ...
                            {!address.street.trim() && (
                                <div className="requirement">• Please enter your street address</div>
                            )}
                            {!isShippingSelected && (
                                <div className="requirement">• Please select a shipping option</div>
                            )}
                        </div>
                    )}
                </div>
