...
export const createOrder = async (userId: string, orderData: Omit<Order, "id" | "createdAt"
    | "updatedAt">): Promise<string> => {
    try {
        // Validate required fields 
        if (!userId) throw new Error("User ID is required");
        if (!orderData.items || orderData.items.length === 0) throw new Error("Order must contain items");
        if (!orderData.payment?.method) throw new Error("Payment method is required");
        if (orderData.total === undefined || orderData.total === null) throw new
        ...
        throw new Error(`Order creation failed: ${error instanceof Error ? error.message :
            String(error)}`);
    }
};

export const updateOrderPaymentStatus = async (userId: string, orderId: string,
    transactionId: string, status: "completed" | "failed") => {
    try {
